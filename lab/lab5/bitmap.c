...
void read_bitmap_metadata(FILE *image, int *pixel_array_offset, int *width, int *height) {
    ...
    //read pixel_array_offset
    fseek(image, 10, SEEK_SET);//move the position
    fread(pixel_array_offset, sizeof(int), 1, image);
    //read the width
    fseek(image, 18, SEEK_SET);//move the position
    fread(width, sizeof(int), 1, image);
    //read the height
    fseek(image, 22, SEEK_SET);//move the position
...
struct pixel **read_pixel_array(FILE *image, int pixel_array_offset, int width, int height) {
    ...
    // for (int i = 0; i < height; i++){
    //     fread(result[i], sizeof(struct pixel), height, image);
    // }

    for (int i = 0; i < height; i++){
        for (int j = 0; j < width; j++){
            fread(&result[i][j], sizeof(struct pixel), 1, image);
        }
    }
